...
void Reducer::store_drop_variable(VariableIndex index) {
    ...
    if (grad.defined()) {
      ...
      if (!grad.is_alias_of(bucket_view)) {
        ...
        if (buc_index.bucket_index != variable_locators_[index.variable_index-1].bucket_index){
          ...
          if(lost_buckets[buc_index.bucket_index].empty()){
            lost_buckets[buc_index.bucket_index]=tensors;
          }
          else{
            int replica_index = 0;
            for (const auto& rep : bucket.replicas) {
              lost_buckets[buc_index.bucket_index][replica_index]*=ef_init+0.1*(num_iterations_>(decay_step*(1-ef_init)/0.1)?((1-ef_init)/0.1):(num_iterations_/decay_step));
              //lost_buckets[buc_index.bucket_index][replica_index]*=0.5;
              lost_buckets[buc_index.bucket_index][replica_index]+=rep.contents;
              replica_index++;
            }
          }
          lost_buckets_bool[buc_index.bucket_index] = true;
        }
        if (gradient_as_bucket_view_) {
  ...
  runGradCallbackForVariable(variable, [&](auto& grad) {
    if (grad.defined()) {
      ...
      if (!grad.is_alias_of(bucket_view)) {
        ...
        if (buc_index.bucket_index != variable_locators_[index.variable_index-1].bucket_index){
          //TORCH_WARN(buc_index.bucket_index);
          local_gradients.push_back(bucket.replicas[0].contents);
        }
...
void Reducer::autograd_hook(VariableIndex index) {
  ...
  const auto v_index = index.variable_index;
  const auto& bucket_index = variable_locators_[v_index];
  std::vector<int>::iterator it;
    ...
    if(partid_of_drop_bucket[bucket_index.bucket_index].size() == cut_strategy[bucket_index.bucket_index])
      return;
  }
...
void Reducer::all_reduce_bucket(Bucket& bucket) {
  ...
  if(!lost_buckets.empty()){
    std::vector<at::Tensor> temp_tensors = lost_buckets[next_bucket_];
    if (keymeng && !temp_tensors.empty() && lost_buckets_bool[next_bucket_]){
      //TORCH_WARN(next_bucket_);
      std::vector<at::Tensor> zero_tensors;
      zero_tensors.reserve(bucket.replicas.size());
      for (const auto& replica : bucket.replicas) {
        zero_tensors.push_back(at::zeros_like(replica.contents));
      }
      lost_buckets[next_bucket_] = zero_tensors;
      lost_buckets_bool[next_bucket_] = false;
      tensors[0]=tensors[0]+temp_tensors[0];
    }
  }
  ...
  if (comm_hook_ == nullptr) {
    ...
    if(it != bucket_drop_id.end() && partid_of_drop_bucket[next_bucket_].size() == cut_strategy[next_bucket_]-1){
      at::TensorOptions options;
    ...
    GradBucket grad_bucket(
        next_bucket_,
        buckets_.size()-bucket_drop_id.size(),
        local_threshold,
        global_threshold,
        tensors[0],
...
void Reducer::mark_bucket_ready(size_t bucket_index) {
  ...
  for (; next_bucket_ < buckets_.size() && buckets_[next_bucket_].pending == 0;
    ...
    if(i>=1 && it != bucket_drop_id.end() && partid_of_drop_bucket[next_bucket_].size() == cut_strategy[next_bucket_]){
...
void Reducer::prepare_for_backward(
  ...
  if(!should_rebuild_buckets()){
    ...
    for (auto id : bucket_drop_id) {
      if(partid_of_drop_bucket[id].size() == cut_strategy[id]){
        buckets_[id].pending = 0;
        if(id == init_id){
          auto f1 = c10::make_intrusive<c10::ivalue::Future>(c10::IntType::get());
          f1->markCompleted(1);
          buckets_[next_bucket_].future_work = f1;
  ...
  for (auto& bucket : buckets_) {
    ...
    if (comm_hook_ == nullptr) {
      ...
      std::vector<int>::iterator it;
	    ...
	    if (it != bucket_drop_id.end() && partid_of_drop_bucket[count_buck].size() == cut_strategy[count_buck]){
        std::vector<at::Tensor> tensors;
        tensors.reserve(bucket.replicas.size());
        for (const auto& replica : bucket.replicas) {
      ...
      else if(it != bucket_drop_id.end() && partid_of_drop_bucket[count_buck].size() != cut_strategy[count_buck]){
        ...
        for (size_t i = 0; i < future_result.size(); i++) {
          auto& replica = bucket.replicas[i];
          int j = 0;
          for(;j < cut_strategy[count_buck];j++){
            std::vector<int>::iterator it2;
            it2 = find(partid_of_drop_bucket[count_buck].begin(), partid_of_drop_bucket[count_buck].end(), j);
            if(it2 == partid_of_drop_bucket[count_buck].end()){
              break;
            }
          }
          at::TensorOptions options;
          ...
          // following the same logic in `initialize_buckets`.
            populate_bucket_views_out(replica, temp);
          }
        }
...
bool Reducer::rebuild_buckets() {
  ...
  partid_of_drop_bucket.reserve(buckets_.size());
  std::vector<at::Tensor> temp_tensors;
  for (int i = 0;i < buckets_.size();i++){
    lost_buckets.push_back(temp_tensors);
    lost_buckets_bool.push_back(false);
    std::vector<int> temp_int_array(cut_strategy[i]);
    partid_of_drop_bucket.push_back(temp_int_array);
  }
  if(droprate > 8){
    ef_init=0.2;
    decay_step=100;
